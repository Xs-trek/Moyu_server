...
import type { ReasoningEffort } from '../adapters/types';
...
export async function handleRest(
  ...
  if (path === '/api/v1/server/info' && method === 'GET') {
    const adapters = await ctx.adapters.refreshStatus();
    sendJson(res, 200, {
  ...
  if (path === '/api/v1/sessions' && method === 'POST') {
    ...
    const stringLimits: Array<[string, number]> = [
      ['profileId', 256], ['model', 128], ['effort', 16], ['title', 256], ['cwd', 4096], ['cliSessionRef', 512],
    ];
  ...
  if (m && method === 'POST') {
    ...
    await ctx.sessions.send(m[1]!, { text: body.text });
    sendJson(res, 202, { ok: true });
    return;
  }
